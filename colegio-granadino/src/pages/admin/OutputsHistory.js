// src/pages/admin/OutputsHistory.js
import React, { useState, useEffect } from "react";
import Layout from "../../components/layout/Layout";
import { getOutputsHistory, getItems } from "../../services/storeService";
...
export default function OutputsHistory() {
  ...
  useEffect(() => {
    Promise.all([getOutputsHistory(), getItems()]).then(([h, i]) => {
      setHistory(h);
      const itemsMap = {};
  ...
  return (
    <Layout>
      <div style={styles.container}>
        <h2 style={styles.title}>{t("nav.outputs_history")}</h2>
          ...
          <table style={styles.table}>
            <thead style={styles.thead}>
              <tr>
                ...
                <th style={styles.th}>{t("common.recipient")}</th>
                <th style={styles.th}>{t("common.reason")}</th>
              </tr>
            </thead>
            <tbody>
              {history.map(row => (
                <tr key={row.id} style={styles.tr}>
                  ...
                  <td style={{ ...styles.td, fontWeight: "700", color: "#c53030" }}>-{row.quantity}</td>
                  <td style={styles.td}>{row.recipient || "—"}</td>
                  <td style={{ ...styles.td, fontSize: 12, opacity: 0.8 }}>{row.reason || "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
